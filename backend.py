...
model.load_state_dict(torch.load('/content/drive/MyDrive/alzeimer.pt'))
...
image = image_loader('/content/moderateDem2.jpg')
out=model(image)
pred_labels = torch.argmax(out[0], 1)
pred_labels
if pred_labels == torch.Tensor([0]).to(device):
  print('Patient Probably has Mild Dementia')
if pred_labels == torch.Tensor([1]).to(device):
  print('Patient Probably has Moderate Dementia')
if pred_labels == torch.Tensor([2]).to(device):
  print('Patient Probably has Non Dementia')
if pred_labels == torch.Tensor([3]).to(device):
  print('Patient Probably has Very Mild Dementia')
